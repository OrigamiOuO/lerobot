...
"""Diffusion Policy with Multi-Modal State Concatenation.

This is a variant of the original Diffusion Policy that supports concatenation of multiple
...
Based on:
- Diffusion Policy: Visuomotor Policy Learning via Action Diffusion (https://huggingface.co/papers/2303.04137)
- ACT Baseline multi-modal state concatenation approach
"""
...
class DiffusionBaselinePolicy(PreTrainedPolicy):
    """Diffusion Policy with multi-modal state features concatenation.
    
    Unlike standard Diffusion Policy, this policy automatically concatenates multiple state observation
    modalities (observation.state, observation.tactile, observation.fsr, etc.) into a single composite 
    state vector for the global conditioning of the UNet.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.diffusion = DiffusionBaselineModel(config, composite_state_dim=self.composite_state_dim)
...
class DiffusionBaselineModel(nn.Module):
    """Diffusion model with composite state conditioning support."""
    
    def __init__(self, config: DiffusionBaselineConfig, composite_state_dim: int = 0):
        super().__init__()
        self.config = config
        self.composite_state_dim = composite_state_dim

        # Build observation encoders (depending on which observations are provided).
        # Use composite state dim instead of single robot_state_feature
        global_cond_dim = composite_state_dim if composite_state_dim > 0 else 0
        
        if self.config.image_features:
            num_images = len(self.config.image_features)
            if self.config.use_separate_rgb_encoder_per_camera:
                encoders = [DiffusionBaselineRgbEncoder(config) for _ in range(num_images)]
                self.rgb_encoder = nn.ModuleList(encoders)
                global_cond_dim += encoders[0].feature_dim * num_images
            else:
                self.rgb_encoder = DiffusionBaselineRgbEncoder(config)
                global_cond_dim += self.rgb_encoder.feature_dim * num_images
        
        if self.config.env_state_feature:
            global_cond_dim += self.config.env_state_feature.shape[0]

        self.unet = DiffusionBaselineConditionalUnet1d(config, global_cond_dim=global_cond_dim * config.n_obs_steps)
    ...
    def _concatenate_state_features(self, batch: dict[str, Tensor]) -> Tensor | None:
        ...
        Returns:
            Concatenated state tensor of shape (B, n_obs_steps, composite_state_dim) or None
        """
        state_parts = []
        for key in self.config.state_feature_keys:
            if key in batch:
    ...
    def _prepare_global_conditioning(self, batch: dict[str, Tensor]) -> Tensor:
        """Encode image features and concatenate them all together along with the composite state vector."""
        # Determine batch size and n_obs_steps from available features
        batch_size = None
        n_obs_steps = None
        
        # Try to get dimensions from available state features
        for key in self.config.state_feature_keys:
            if key in batch:
        ...
        # Fallback to images or env state
    ...
    def generate_actions(self, batch: dict[str, Tensor], noise: Tensor | None = None) -> Tensor:
        ...
        {
            "observation.state": (B, n_obs_steps, state_dim)  # or other state features
        ...
        """
        # Get batch_size and n_obs_steps from available features
        batch_size = None
        n_obs_steps = None
    ...
    def compute_loss(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        """
        # Input validation - check we have at least one state feature or other conditioning
        assert ACTION in batch
        assert "action_is_pad" in batch
        
        # Get n_obs_steps from available features
        n_obs_steps = None
        for key in self.config.state_feature_keys:
            if key in batch:
                n_obs_steps = batch[key].shape[1]
...
class SpatialSoftmax(nn.Module):
    ...
    def __init__(self, input_shape, num_kp=None):
        ...
        # Use numpy for linspace to match original behavior
...
class DiffusionBaselineRgbEncoder(nn.Module):
...
class DiffusionBaselineConditionalUnet1d(nn.Module):
