...
class CrossModalAttentionFusion(nn.Module):
    """Attention-based feature fusion over selected observation modality tokens.

    The module consumes ``modality_features`` with tensors of shape (B, S, D_i),
    projects only the selected modalities to a common attention dimension, and
    returns one fused feature per observation step with shape (B, S, D_attn).

    The attended modalities are intentionally configurable so expensive/weak
    streams can stay as ordinary concatenated conditioning features instead of
    participating in cross-attention.
    """
    ...
    def __init__(
        ...
        key_value_names: tuple[str, ...] | list[str],
        ...
        self.key_value_names = tuple(key_value_names)
        if len(self.query_names) == 0:
            raise ValueError("`query_names` must contain at least one modality.")
        if fusion_type != "self_attention" and len(self.key_value_names) == 0:
            raise ValueError(f"`{fusion_type}` requires at least one key/value modality.")

        if fusion_type == "self_attention":
            self.attended_names = self._unique_names(self.query_names + self.key_value_names)
            if len(self.attended_names) < 2:
                raise ValueError(
                    "`self_attention` requires at least two selected modality tokens. "
                    f"Got {self.attended_names}."
                )
        elif fusion_type in {"cross_v2t", "cross_t2v", "bidirectional_cross"}:
            missing_query = [name for name in self.query_names if name not in modality_input_dims]
            missing_kv = [name for name in self.key_value_names if name not in modality_input_dims]
            if missing_query or missing_kv:
                raise ValueError(
                    f"`{fusion_type}` requested unavailable attention modalities. "
                    f"missing_query={missing_query}, missing_key_value={missing_kv}, "
                    f"available={list(modality_input_dims)}."
                )
            self.attended_names = self._unique_names(self.query_names + self.key_value_names)
        else:
            raise ValueError(f"Unsupported attention fusion type {fusion_type}.")

        missing_attended = [name for name in self.attended_names if name not in modality_input_dims]
        if missing_attended:
            raise ValueError(
                "Attention requested unavailable modality tokens: "
                f"{missing_attended}. Available modalities are {list(modality_input_dims)}."
            )
    ...
    def forward(self, modality_features: dict[str, Tensor]) -> Tensor:
        ...
        if self.fusion_type == "cross_v2t":
            ...
            # Reverse the configured direction while keeping the same selected sets.
