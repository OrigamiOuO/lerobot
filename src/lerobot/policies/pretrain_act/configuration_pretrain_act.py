...
@PreTrainedConfig.register_subclass("pretrain_act")
@dataclass
class PretrainACTConfig(PreTrainedConfig):
    """ACT-style policy using a pretrained sparse point-cloud temporal encoder.

    Same observation conditioning as PretrainDiffusion:
    ...
    The action generator is a Transformer Decoder (ACT-style) with learned position
    queries that cross-attend to the conditioning vector, directly outputting action
    chunks without iterative denoising.
    """
    ...
    # --- ACT-specific transformer decoder config ---
    # Transformer decoder that cross-attends to the global conditioning vector.
    ...
    def global_cond_dim(self) -> int:
        """Total conditioning dim fed into the transformer decoder."""
        if self.ablation_mode == "full":
