...
"""Diffusion Policy V2 with Multi-Modal State + Sparse PC Conditioning.

Extends DiffusionBaseline by additionally encoding ``observation.sparse_pc``
(44×4 point cloud) with a lightweight MLP and injecting the result into the
UNet global conditioning.
"""
...
from lerobot.policies.diffusion_baseline_v2.configuration_diffusion_baseline_v2 import DiffusionBaselineV2Config
...
class DiffusionBaselineV2Policy(PreTrainedPolicy):
    """Diffusion Policy with multi-modal state + sparse_pc encoding."""

    config_class = DiffusionBaselineV2Config
    name = "diffusion_baseline_v2"

    def __init__(
        self,
        config: DiffusionBaselineV2Config,
        **kwargs,
    ):
        ...
        self._queues = None
        self.diffusion = DiffusionBaselineV2Model(config, composite_state_dim=self.composite_state_dim)
        self.reset()
...
class DiffusionBaselineV2Model(nn.Module):
    """Diffusion model with composite state + sparse_pc conditioning."""
    
    def __init__(self, config: DiffusionBaselineV2Config, composite_state_dim: int = 0):
        super().__init__()
        self.config = config
        ...
        # --- Sparse PC encoder ---
        if self.config.include_sparse_pc_in_cond:
            self.sparse_pc_encoder = SparsePCEncoder(
        ...
        if self.config.image_features:
            ...
            if self.config.use_separate_rgb_encoder_per_camera:
                encoders = [DiffusionBaselineV2RgbEncoder(config) for _ in range(num_images)]
                self.rgb_encoder = nn.ModuleList(encoders)
                global_cond_dim += encoders[0].feature_dim * num_images
            else:
                self.rgb_encoder = DiffusionBaselineV2RgbEncoder(config)
                global_cond_dim += self.rgb_encoder.feature_dim * num_images
        
        if self.config.env_state_feature:
            global_cond_dim += self.config.env_state_feature.shape[0]

        self.unet = DiffusionBaselineV2ConditionalUnet1d(config, global_cond_dim=global_cond_dim * config.n_obs_steps)
    ...
    def _concatenate_state_features(self, batch: dict[str, Tensor]) -> Tensor | None:
        """Concatenate multiple state features (sparse_pc excluded — handled by separate encoder)."""
        state_parts = []
    ...
    def _prepare_global_conditioning(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        # Fallback: try sparse_pc, images or env state
        ...
        # --- Encode sparse_pc (or use zero fallback if not available in batch) ---
        if self.sparse_pc_encoder is not None:
            if OBS_SPARSE_PC in batch:
                # sparse_pc shape: (B, T, N, 4) — flatten batch&time, encode, restore
                spc = batch[OBS_SPARSE_PC]  # (B, n_obs_steps, 44, 4)
                B_sp, T_sp = spc.shape[:2]
                spc_flat = spc.reshape(B_sp * T_sp, *spc.shape[2:])  # (B*T, 44, 4)
                spc_feat = self.sparse_pc_encoder(spc_flat)           # (B*T, embed_dim)
                spc_feat = spc_feat.reshape(B_sp, T_sp, -1)           # (B, T, embed_dim)
            else:
                # Fallback: zero embedding when sparse_pc not in batch
...
class DiffusionBaselineV2RgbEncoder(nn.Module):
    """Encodes an RGB image into a 1D feature vector.

    Includes the ability to normalize and crop the image first.
    """

    def __init__(self, config: DiffusionBaselineV2Config):
        super().__init__()
...
class DiffusionBaselineV2ConditionalUnet1d(nn.Module):
    """A 1D convolutional UNet with FiLM modulation for conditioning.

    Note: this removes local conditioning as compared to the original diffusion policy code.
    """

    def __init__(self, config: DiffusionBaselineV2Config, global_cond_dim: int):
        super().__init__()
...
class SparsePCEncoder(nn.Module):
    """Lightweight point-wise MLP encoder for the sparse point cloud (44×4).

    Architecture (per-point):
        Linear(4 → 32) → ReLU → Linear(32 → 64) → ReLU

    Then mean-pool over the N points and project to ``embed_dim``.

    Input:  (B, N, C)   where N=44, C=4 (xyz + force)
    Output: (B, embed_dim)
    """

    def __init__(self, num_points: int = 44, point_dim: int = 4, embed_dim: int = 64):
        super().__init__()
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        h = self.mlp(x_flat)                                      # (B*N, 64)
        h = h.reshape(B, self.num_points, -1)                     # (B, N, 64)
        h = h.mean(dim=1)                                         # (B, 64)  mean pool
