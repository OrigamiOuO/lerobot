...
"""Configuration for DiffusionBaselineV3Policy — PointNet-style sparse PC encoder."""
...
@PreTrainedConfig.register_subclass("diffusion_baseline_v3")
@dataclass
class DiffusionBaselineV3Config(PreTrainedConfig):
    """Diffusion policy with PointNet-style sparse PC encoding.

    Compared to V2, the V3 encoder uses:

    - **Max-pooling** (PointNet-style) instead of mean-pooling, preserving
      the most salient per-point features rather than averaging them away.
    - **Deeper per-point MLP** (4→64→128 vs 4→32→64) with LayerNorm for
      better feature extraction and training stability.
    ...
    """Output dimension (added to UNet global_cond_dim)."""
    sparse_pc_num_points: int = 44
    sparse_pc_point_dim: int = 4
    sparse_pc_hidden_dims: tuple[int, ...] = (64, 128)
    """Hidden dimensions of the per-point MLP (default 64→128, deeper than V2)."""
